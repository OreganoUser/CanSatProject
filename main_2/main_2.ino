...
void loop() {
  if (dropped) {
    ...
    write_data_to_file(logfile_name, number_of_arrays, data_arrays, data_array_sizes, precisions);
    //send_data_rf95(rf95, iteration_counter, data_arrays, data_array_sizes, number_of_arrays, precisions);

    iteration_counter++;

    //Serial.println("iiiiiiiiiiiiiiiii");

    aimSatellite();   // Check for new target coordinates.
    if (targetSet) { // Coordinaten sou aginn: AIM 48.8584 2.2945
      adjustDirection(targetLat, targetLon);
    }
  }
}
